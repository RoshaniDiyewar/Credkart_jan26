...
def browser_setup(request): # function
    ...
    if browser == "chrome":
        ...
        chrome_options.add_experimental_option("prefs", {
            ...
            # This turns OFF Chrome's built-in Password Manager feature.
            # Password Manager related popups (Save password / breach warning) will not come.
            "profile.password_manager_leak_detection": False  # to avoid Chrome browser alert for save password
        }
                                               )
        driver = webdriver.Chrome(options=chrome_options)
        # Launch Chrome using the above options.
...
def pytest_html_results_summary(prefix, summary, postfix):
    prefix.extend([
        ...
        "<p><strong>Framework:</strong> Pytest + Selenium</p>",
        "<p><strong>Execution Type:</strong> Regression</p>"
    ])
"""